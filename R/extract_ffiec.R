...
extract_ffiec_tsv <- function(db_connector, zf, sch) {
  ...
  if (db_not_empty) {
    matching_sch_in_db <- dplyr::filter(
      scheds_in_db,
...
parse_ffiec_tsv <- function(sch_unzipped) {
  ...
  report_date <-
    stringr::str_extract(sch_file, "[[:digit:]]{8}") %>%
    lubridate::mdy()
...
read_ffiec_filename <- function(sch) {
  dt <-
    stringr::str_extract(basename(sch), "[[:digit:]]{8}") %>%
    lubridate::mdy()
  sch_info <- c(
    report_date = as.character(dt),
...
read_ffiec_part_codes <- function(sch) {
  ...
  if (is.na(part_codes[1, 3])) {
    return(c(part_num = 1, part_of = 1))
  }
  return(c(
    part_num = as.numeric(part_codes[1, 4]),
