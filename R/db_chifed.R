...
fetch_chifed_codebook_by_varcode_ids <- function(db_connector, varcode_ids) {
  ...
  db_res <- 
    ...
    paste('SELECT A."ID" AS "VAR_CODE_ID", B.* FROM "VAR_CODES" A',
          'INNER JOIN "CHIFED.CODEBOOK" B',
          'ON A."VAR_CODE" = B."VAR_CODE"',
          'WHERE A."ID" IN (', ., ')') %>%
    DBI::dbSendQuery(db_conn, .)
  df_out <- 
    DBI::dbFetch(db_res) %>% as_tibble() %>%
...
available_chifed_varcodes <-
  function(db_connector, min_date = NULL, max_date = NULL) {
    ...
    df_details <-
      ...
      mutate(MNEMONIC  = substr(VAR_CODE, 1, 4),
    ...
    df_details %<>%
      select(VAR_CODE_ID, VAR_CODE, REPORTING_FORM,
             !any_of(c('MNEMONIC', 'ITEM_CODE',
